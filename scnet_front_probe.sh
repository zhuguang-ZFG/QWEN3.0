set -e
cd /tmp
rm -rf scnet_front && mkdir scnet_front && cd scnet_front
python3 - <<'PY'
...
terms=['file/upload/url','file/parse','textFile','imageFile','fileId','uploadUrl','download/url','parse/']
for path in Path('.').glob('*'):
    ...
    if not hits:
        continue
    print('\nFILE', path.name, 'HITS', hits)
    for term in hits:
        index=text.find(term)
        snippet=text[max(0,index-900):index+1600].replace('\n',' ')
        print('TERM', term, 'AT', index)
        print(snippet[:2600])
PY
