...
弱后端在并行工具调用时经常出现：
  - JSON arguments 格式错误（缺引号、trailing comma、未转义字符）
  - tool_call_id 遗漏或重复
  - 参数混淆（把 A 工具的参数写到 B 工具）
  - 幻觉出不存在的工具名

本模块在 SSE 流中检测并拆分并行工具调用，确保弱后端也能正确处理。

核心功能:
  1. detect_parallel_tool_calls() — 检测 chunk 中的并行 tool_calls
  2. split_parallel_tool_calls() — 拆分为顺序调用列表
  3. build_sequential_tool_prompt() — 弱后端专用"逐个调用"提示
  4. validate_tool_call_args() — 校验 JSON arguments
  5. repair_tool_call_json() — 修复常见 JSON 格式错误
  6. inject_tool_ordering_hint() — 注入工具调用顺序建议
...
_OPENCODE_BUILTIN_TOOLS = frozenset({
    "invalid", "question", "shell", "read", "glob", "grep",
    "edit", "write", "task", "webfetch", "todo_write",
    "web_search", "skill", "apply_patch", "lsp",
...
_TOOL_ORDER_PRIORITY: dict[str, int] = {
    "read": 0, "glob": 0, "grep": 0,  # 信息获取最先
    "task": 1, "todo_write": 1,  # 规划和子任务
    "webfetch": 2, "web_search": 2, "lsp": 2,  # 外部信息
    "shell": 3,  # 执行/测试
    "question": 4,  # 向用户提问
    "edit": 5, "write": 5, "apply_patch": 5,  # 文件修改最后
    "skill": 6, "plan_enter": 6, "plan_exit": 6,  # 元操作
}
...
def build_sequential_tool_prompt(tools: list[dict] | None = None) -> str:
    ...
    if tools:
        ...
        if read_tools:
            lines.append(
                f"- Recommended first: read/gather info via "
                + ", ".join(f"`{t}`" for t in read_tools)
            )
        if edit_tools:
            lines.append(
                f"- Then modify: " + ", ".join(f"`{t}`" for t in edit_tools)
            )
