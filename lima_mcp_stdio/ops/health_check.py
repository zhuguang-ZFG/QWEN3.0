...
def _health_summary(shost, label, run_ssh, results):
    """Collect summary health codes for one server."""
    health = run_ssh(
        shost,
...
def _health_detail(shost, label, run_ssh, results):
    """Collect detailed health info for one server."""
...
def tool_health_check(
    ...
    for shost, info in _filter_servers(servers, host).items():
        label = info["label"]
        if summary:
            _health_summary(shost, label, run_ssh, results)
        else:
            _health_detail(shost, label, run_ssh, results)
