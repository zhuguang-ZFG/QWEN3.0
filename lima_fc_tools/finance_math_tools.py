...
@tool(
    "get_crypto_price",
    "Run the get_crypto_price utility.",
    {
        "properties": {"coin": {"description": "Coin identifier.", "type": "string"}},
        "required": ["coin"],
        "type": "object",
    },
)
async def _crypto(coin: str) -> dict:
    try:
        ...
        _log.warning("crypto price fetch failed for %s: %s", coin, exc)
    return {"coin": coin, "error": "Price unavailable (blocked in China)"}


@tool(
    "get_stock_price",
    "Run the get_stock_price utility.",
    {"properties": {"code": {"description": "Stock code.", "type": "string"}}, "required": ["code"], "type": "object"},
)
    ...
    try:
        ...
        if isinstance(r, str) and "," in r:
            ...
            return {
                ...
                "name": name,
                "open": parts[1],
                "close_yesterday": parts[2],
                "current": parts[3],
                "high": parts[4],
                "low": parts[5],
            }
    except Exception as exc:
        _log.warning("stock price fetch failed for %s: %s", code, exc)
...
@tool(
    ...
    {
        "properties": {"expression": {"description": "Math expression.", "type": "string"}},
        "required": ["expression"],
        "type": "object",
    },
)
...
@tool(
    "convert_timezone",
    "Run the convert_timezone utility.",
    {
        "properties": {
            "from_tz": {"description": "Source timezone.", "type": "string"},
...
@tool(
    "convert_units",
    "Run the convert_units utility.",
    {
        "properties": {
            "from_unit": {"description": "Source unit.", "type": "string"},
