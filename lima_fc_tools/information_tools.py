...
async def _weather(city: str) -> dict:
    ...
    if isinstance(r, dict) and 'current_condition' in r:
        ...
        return {'city': city, 'temp_c': cc.get('temp_C'), 'feels_like_c': cc.get('FeelsLikeC'), 'humidity': cc.get('humidity') + '%', 'weather': cc.get('lang_zh', [{}])[0].get('value', cc.get('weatherDesc', [{}])[0].get('value', '')), 'wind_kmph': cc.get('windspeedKmph'), 'wind_dir': cc.get('winddir16Point'), 'visibility_km': cc.get('visibility'), 'uv_index': cc.get('uvIndex')}
    return {'error': 'unavailable', 'raw': str(r)[:200]}

@tool('get_air_quality', 'Run the get_air_quality utility.', {'properties': {'city': {'description': 'City name.', 'type': 'string'}},
 'required': ['city'],
 'type': 'object'})
async def _air_quality(city: str) -> dict:
    ...
    if isinstance(r, dict) and 'current_condition' in r:
        cc = r['current_condition'][0]
        return {'city': city, 'humidity': cc.get('humidity'), 'visibility_km': cc.get('visibility'), 'uv_index': cc.get('uvIndex'), 'cloud_cover': cc.get('cloudcover')}
    return {'error': 'unavailable'}

@tool('get_ip_info', 'Run the get_ip_info utility.', {'properties': {'ip': {'description': 'Public IP address.', 'type': 'string'}},
 'required': ['ip'],
 'type': 'object'})
...
@tool('get_exchange_rate', 'Run the get_exchange_rate utility.', {'properties': {'amount': {'default': 1, 'description': 'Amount to convert.', 'type': 'number'},
...
@tool('get_holiday', 'Run the get_holiday utility.', {'properties': {'date': {'description': 'Date in YYYY-MM-DD format.', 'type': 'string'}},
 'required': ['date'],
 'type': 'object'})
...
@tool('get_hot_search', 'Run the get_hot_search utility.', {'properties': {'platform': {'description': 'Source platform.',
                             'enum': ['weibo', 'baidu', 'zhihu', 'douyin', 'toutiao', 'bili'],
                             'type': 'string'}},
 'required': ['platform'],
 'type': 'object'})
async def _hot_search(platform: str) -> dict:
    urls = {'weibo': 'https://weibo.com/ajax/side/hotSearch', 'baidu': 'https://top.baidu.com/api/board?platform=wise&tab=realtime', 'zhihu': 'https://www.zhihu.com/api/v3/feed/topstory/hot-lists/total?limit=10', 'douyin': 'https://www.douyin.com/aweme/v1/web/hot/search/list/', 'toutiao': 'https://www.toutiao.com/hot-event/hot-board/?origin=toutiao_pc', 'bili': 'https://api.bilibili.com/x/web-interface/ranking/v2?rid=0&type=all'}
    try:
...
@tool('get_news', 'Fetch GNews headlines or keyword search results.', {'properties': {'category': {'default': 'general',
...
async def _news(keyword: str='', category: str='general', lang: str='zh') -> dict:
    ...
    if keyword:
        ...
        r = await _get('https://gnews.io/api/v4/top-headlines', {'category': category, 'lang': lang, 'max': 5, 'apikey': key})
    articles = r.get('articles', []) if isinstance(r, dict) else []
    return {'articles': [{'title': a.get('title'), 'description': a.get('description', '')[:100], 'source': a.get('source', {}).get('name', '')} for a in articles[:5]]}

@tool('translate_text', 'Translate text to a target language.', {'properties': {'text': {'description': 'Input text.', 'type': 'string'},
...
@tool('get_oil_price', 'Run the get_oil_price utility.', {'properties': {'province': {'default': 'Guangdong',
...
@tool('get_express_tracking', 'Run the get_express_tracking utility.', {'properties': {'number': {'description': 'Tracking number.', 'type': 'string'}},
 'required': ['number'],
 'type': 'object'})
...
@tool('get_history_today', 'Run the get_history_today utility.', {'properties': {'day': {'description': 'Day number.', 'type': 'integer'},
...
@tool('get_idiom', 'Run the get_idiom utility.', {'properties': {'word': {'description': 'Word to look up.', 'type': 'string'}},
 'required': ['word'],
 'type': 'object'})
...
@tool('generate_qrcode', 'Run the generate_qrcode utility.', {'properties': {'text': {'description': 'Input text.', 'type': 'string'}},
 'required': ['text'],
 'type': 'object'})
...
@tool('shorten_url', 'Run the shorten_url utility.', {'properties': {'url': {'description': 'URL.', 'type': 'string'}},
 'required': ['url'],
 'type': 'object'})
