...
def with_stable_urls(image: dict[str, Any], request: Request) -> dict[str, Any]:
...
async def list_images_for_app(
    ...
    *,
    ...
    return [with_stable_urls(image, request) for image in images]
...
async def get_cached_image_bytes(
    ...
    cache_key = f"{account_id}:{image_id}"
    now = time.time()
...
def proxy_cache_headers(*, query_token_auth: bool, for_file: bool = False) -> dict[str, str]:
    """Cache policy for gallery proxy responses."""
    if query_token_auth or for_file:
