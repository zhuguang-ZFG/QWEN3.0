...
async def real_stream_chunks_async(backend_name: str, msgs: list, max_tokens: int = 4096, ide: str = "unknown"):
...
async def speculative_stream_chunks(
    ...
    def _predict(q, m, sp, ide_source):
        return v3_predict(q, m, system_prompt=sp, ide=ide_source, preferred_backend=prefer)
