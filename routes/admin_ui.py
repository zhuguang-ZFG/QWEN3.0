...
<script>
var state={stats:null,logs:[],backends:[],model:null,traces:[],agents:[],_poolFilter:'all',_loading:false};
...
var panelLoaders={overview:loadStats,traffic:loadLogs,backends:loadBackends,retrieval:loadRetrieval,model:loadModel,agents:loadAgents};
...
async function refreshAll(){try{setRefreshStatus('刷新中...');await Promise.allSettled([loadStats(),loadLogs(),loadBackends(),loadModel(),loadRetrieval(),loadAgents()]);setRefreshStatus('已刷新 '+new Date().toLocaleTimeString())}catch(e){setRefreshStatus('刷新失败: '+e.message);console.error('refreshAll error:',e)}}
...
async function triggerRetrain(){if(!confirm('确认触发 auto_retrain.py --force ?'))return;try{toast('正在触发重训...');var r=await json('/admin/api/retrain',{method:'POST'});toast('训练任务: '+(r.job_id||r.status||'已启动'))}catch(e){toast('触发失败: '+e.message,'err')}}
