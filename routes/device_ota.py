...
def _ota_status_for_device(device_id: str, current_version: str) -> dict:
    ...
    canary_version = _canary.deployed_version
    canary_firmware = _canary.firmware
    gradual_version = _gradual.version
    gradual_firmware = _gradual.firmware

    selected_in_canary = _canary.is_canary(device_id)
    selected_in_gradual = _gradual.is_device_selected(device_id)
...
async def app_start_ota(request: Request, authorization: str = Header(default="")):
    ...
    if rollback:
        if _canary.is_canary(device_id):
            _canary.remove_canary_device(device_id)
        status = _ota_status_for_device(device_id, current_version)
    ...
    if not _canary.deployed_version:
