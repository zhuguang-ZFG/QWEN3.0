...
async def store_memory(req: StoreMemoryRequest) -> dict:
    ...
    try:
        from session_memory.store import store_memory
...
@router.get("/context")
async def get_routing_context(scenario: str = "") -> dict:
    ...
    try:
        ...
        ctx = hmem.get_context_for_routing(scenario)
