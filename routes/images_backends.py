...
_XMIAOM_URL = "https://ai.xmiaom.com/v1/chat/completions"


async def _generate_via_xmiaom(prompt: str, size: str) -> list[dict]:
    """Call xmiaom gpt-image-2 and return image URL objects."""
    if not backend_config.XMIAOM_API_KEY:
        _log.warning("xmiaom image generation disabled: XMIAOM_API_KEY not set")
        return []

    payload = _build_xmiaom_payload(prompt, size)
    headers = {"Authorization": f"Bearer {backend_config.XMIAOM_API_KEY}", "Content-Type": "application/json"}
    started = time.time()
    try:
        async with httpx.AsyncClient(timeout=_XMIAOM_IMAGE_TIMEOUT) as client:
            response = await client.post(_XMIAOM_URL, content=payload, headers=headers)
            response.raise_for_status()
            data = response.json()
