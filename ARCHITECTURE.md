...
### 15.2 快速质量评分（_quick_score）

不调用外部 API，纯本地规则评分（0ms 延迟）：
- 长度分（0.3权重）：100-2000字符=1.0，过短/过长扣分
- 格式分（0.3权重）：代码块闭合+0.4，数字/单位+0.3，步骤编号+0.3
- 完整性分（0.2权重）：不含"抱歉"/"无法"/"不确定"=1.0
- 相关性分（0.2权重）：回答中包含问题关键词的比例
...
```json
{
  "query": "步进电机失步怎么排查",
  "answer": "检查驱动器电流...",
  "backend": "longcat_thinking",
  "intent": "cnc_trouble",
  "complexity": 0.8,
  "quality_score": 0.85,
  "routing_correct": true,
  "logged_at": "2026-05-18T..."
}
```

### 15.4 路由训练数据自动生成

当 `D:/GIT/data/distill_queue/pending/` 积累 >= 1000 条带评分的日志时：
1. score >= 0.8 的条目 → 正样本（路由决策正确）
2. score < 0.6 的条目 → 负样本（路由决策错误，需要换后端）
3. 自动生成路由训练数据：`(query, intent) → backend`
4. 写入 `D:/GIT/data/training_data/round8_routing_feedback.json`
...
第二层：本地模型分类（0.3-0.5s，覆盖30%请求）
  模糊请求 → Qwen3-8B 输出 JSON 路由决策

第三层：置信度级联（1-2s，覆盖10%最难请求）
  本地模型不确定时 → 先试免费模型
  → 质量不够 → 升级到付费模型
```
