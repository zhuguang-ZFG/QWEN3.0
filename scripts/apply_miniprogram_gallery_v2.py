...
def write_format_bytes() -> None:
    path = MM / "src/utils/formatBytes.ts"
    path.write_text(
...
def write_use_gallery_list() -> None:
    path = MM / "src/pages/v2/device-detail/composables/useGalleryList.ts"
    path.write_text(
...
export function useGalleryList() {
  ...
  async function loadGallery(reset = true) {
    ...
    try {
      ...
      totalCount.value = res.total ?? res.count
      if (reset) {
...
def write_gallery_api() -> None:
    path = MM / "src/api/gallery/gallery.ts"
    path.write_text(
...
def write_gallery_types() -> None:
    path = MM / "src/api/gallery/types.ts"
    path.write_text(
...
def write_gallery_panel() -> None:
    path = MM / "src/pages/v2/device-detail/components/gallery-panel.vue"
    path.write_text(
