...
try:
    ...
    stdin, stdout, stderr = ssh.exec_command(f"cp {REMOTE_DIR}/routes/admin_ui.py {REMOTE_DIR}/routes/admin_ui.py.bak.m20")
    ...
    print("\n🎨 Admin panel smoke test:")
    stdin, stdout, stderr = ssh.exec_command(
        "curl -sf -o /dev/null -w '%{http_code}' https://lima.257339.xyz/admin"
    )
    ...
    print("🔧 Rollback: cp /opt/lima-router/routes/admin_ui.py.bak.m20 /opt/lima-router/routes/admin_ui.py && systemctl restart lima-router.service")
