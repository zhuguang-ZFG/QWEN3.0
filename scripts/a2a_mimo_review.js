#!/usr/bin/env node
/**
 * One-shot: send code review task to MiMo via local A2A MCP bridge (streamable-http).
 * Usage: node scripts/a2a_mimo_review.js
 *
...
const REVIEW_PROMPT = `请对以下项目进行独立、只读的 code review（不要修改任何文件，不要读取或输出 .env 中的任何密钥/密码明文）。
...
审查范围（优先）:
1. 语音栈: device_voice/, routes/device_app_voice.py, routes/device_app_voice_ws.py, voice_app_ws_ticket.py
2. access_guard.py 中 WS 鉴权与 ticket 消费逻辑
3. 安全: ticket TTL、限流、异常处理、无硬编码密钥（仅检查代码，勿读 .env 内容）
4. 测试: tests/test_device_app_voice*.py
5. 架构: 退役路径、模块边界

审查输出格式（必须遵守）:
## 总体评价
## 优点（最多 6 条）
## 问题（P0/P1/P2，每条含：文件路径、问题描述、是否可复现、建议）
## 测试盲区
## 与常见审查偏差的自我校验（列出你可能误判的点）

请基于实际代码阅读，给出具体可操作反馈。避免泛泛而谈。`;
  ...
  async connect() {
    ...
    const initBody = {
      ...
      params: {
        ...
        capabilities: {},
        clientInfo: { name: "dlc-mimo-review", version: "1.0.0" },
      },
    };
...
async function main() {
  ...
  console.error(`[a2a] Sending review to MiMo (${MIMO_URL}), timeout ${TIMEOUT_MS}ms...`);
