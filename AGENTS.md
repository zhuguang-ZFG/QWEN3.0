...
| Intent classify | `routing_classifier.py` | `smart_router.classify` |
| Backend pools | `router_v3.py` (`POOLS` dict) | — |
| Backend ranking | `routing_selector.py` | — |
| Backend execution | `routing_executor.py` | — |
| HTTP transport | `http_caller.py` (→ `http_sync`/`http_async`/`http_stream`) | `router_http.py` (urllib) |
| Health/cooldown | `health_tracker.py` | `router_circuit_breaker.py` |
...
| Provider Probe | `provider_probe/`, `provider_automation/`, `backends_registry/` | Auto-discovery of new AI providers |
...
- `smart_router.py` — **legacy facade**; new code imports from underlying modules directly
...
- **HTTP client:** httpx (replacing legacy urllib in `router_http.py`)
...
| Module | Status |
|--------|--------|
| `smart_router.py` chat path | Legacy facade; use `routing_engine.route()` |
| `router_http.py` direct calls | Legacy urllib; use `http_caller` |
