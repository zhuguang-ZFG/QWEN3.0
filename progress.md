...
## 2026-07-01 chat-web 匿名简单聊天请求分流到阿里云 pilot

- **目标**：让 `app.donglicao.com` / `chat.donglicao.com/chat/` 的匿名简单聊天请求走阿里云 `lima-router-pilot`（仅免费后端），降低京东云主节点负载。
- **实现**：
  - 新增 `chat-web/js/app-config.js`：提供 `shouldUsePilot(path, body)` 判定规则（匿名 + 默认模型 `lima`/`lima-1.3` + 无 tools/图片）。
  - 修改 `chat-web/chat-api.js`：`sendMessage()` 与 `generateImage()` 通过 `LiMaConfig.getApiUrl()` 选择 endpoint。
  - 修改 `chat-web/index.html`：CSP `connect-src` 增加 `https://aliyun.donglicao.com`。
  - 更新 `scripts/deploy_chat_web.py` 文件列表，包含 `js/app-config.js`。
  - `.gitignore` 增加 `chat-web/dist/`（由 `hash-assets.mjs` 生成）。
- **部署**：
  - 通过 `scripts/deploy_unified.py --files` 将变更文件同步到京东云 `/opt/lima-router/chat-web`（FastAPI 静态文件回源）。
  - `git push` 触发 GitHub Actions `Deploy Chat Web` workflow，自动构建 hashed assets 并部署到 Cloudflare Pages（`app.donglicao.com`）。
- **验证**：
...
- **验证**：
  - esp32S_XYZ CI 全绿通过（run 28326071680）：U8 firmware build 6m51s、U1 firmware build 1m2s、native tests、schema、GPIO、Python unit、fake integration、manager mobile、markdown link check 全部 ✓
  - CI 静态测试 115 passed（无回归）；`dlc_motor_control_p1.h` 不依赖任何已删配置（引用检查通过）。
...
- **提交**：esp32S_XYZ `af04257` `feat(u1): slim Machines to dlc-only and document OTA decision`；父仓库子模块指针 `af19a217`。
...
- **关键结果**：
  ...
  - 提交并推送：
    ...
    - `ebe78a83` 同步 esp32S_XYZ 子模块：U8 固件修复 custom message cJSON 内存泄漏
- **验证**：
...
- **关键改动**：
  ...
  - `scripts/check_code_size.py`：新增命令行参数 `paths`，无参数时仍可全量或 `--git-tracked` 检查；有参数时仅检查指定文件。
  - `scripts/run_pre_commit_check.py`：`quick_commands` 与 `run_code_size_check` 均将已计算的 `changed_paths`（staged 文件）传给对应脚本，使本地提交只检查待提交改动。
- **验证**：
  - 全量模式：`python scripts/run_ruff_check.py` / `python scripts/check_code_size.py --git-tracked` → 仍检查全部 tracked 文件，均通过。
  - Staged 模式：`git add` 相关脚本后运行 `python scripts/run_pre_commit_check.py` → ruff 与 code_size 均仅检查 staged 文件，耗时秒级；`git diff --cached --check` 与 `py_compile` 均通过，无历史文件警告。
- **影响**：CI/无参数调用行为不变；本地 pre-commit 大幅提速，输出干净，不再因全量扫描超时或被历史警告干扰。
...
- **全量验证**：
  ...
  - Navbar 语言切换按钮在中英文页面间可正常跳转。
  - 已推送至 `origin improve/20260625-phase-a`。
- **关键结果**：
...
- **关键结果**：
  ...
  - 点击设备跳转 `devices.html?id=...`，便于进一步管理。
...
## 2026-06-25 完成 Phase B P1 B-4：设备管理页

- **目标**：按 `docs/LIMA_IMPROVEMENT_PLAN_20260625_V2.md` 执行 Phase B P1 B-4，为用户控制台新增已绑定设备列表、详情抽屉、实时状态与解绑能力。
- **关键结果**：
  - 新增 `chat-web/devices.html` + `js/devices.js`：
    - 卡片式设备列表，展示在线/离线/运行中状态、型号、固件版本。
    - 右侧抽屉详情，显示设备信息、最近 5 条任务、实时 WebSocket 状态推送。
    - 添加设备弹窗（SN + 激活码）调用 `POST /device/v1/app/devices/bind`。
    - 解绑确认弹窗调用 `POST /device/v1/app/devices/{id}/unbind`。
...
- **验证**：
  ...
  - `ruff check` / `pyright` 修改 Python 文件 clean。
- **部署**：`scripts/deploy_chat_web.py` 因本地环境缺少 `LIMA_DEPLOY_PASS` 且 paramiko 无法解析当前 SSH 私钥而未能自动部署；已准备好文件，待配置密码后一键同步。
- **Git**：worktree 分支 `improve/20260625-phase-a`，待提交。
...
- **Git**：worktree 分支 `improve/20260625-phase-a`，commit `51ffdfec` + `98829dbe` + `8aa7168c`。
- **部署**：通过 VPS 部署密钥完成生产部署。
  - `https://www.donglicao.com/pricing.html` 200 OK。
  - `https://www.donglicao.com/docs/` 200 OK（路径 `/docs/`；`docs.donglicao.com` 子域 DNS 未指向 VPS，待 DNS 配置后可切子域）。
  - nginx 配置已更新：`/etc/nginx/conf.d/www.donglicao.com.conf`，原配置已备份。
