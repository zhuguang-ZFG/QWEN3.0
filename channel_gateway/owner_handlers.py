...
def build_owner_code_task_handler() -> Callable:
    """Owner /code-task <goal> — create an agent task for Agent Worker."""
    ...
    def handler(user_id: str, goal: str) -> str:
        ...
        try:
            ...
            return (
                ...
                f"Run the generic Agent Worker against `/agent/tasks` to claim it."
            )
        except ImportError:
...
def build_owner_artifact_handler() -> Callable:
    """Owner /artifact <task_id> — show Agent Worker artifact summary."""
