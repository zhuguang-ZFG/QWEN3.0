...
async def _vision_route(messages: list, max_tokens: int = 4096, ide: str = "unknown") -> dict | None:
    ...
    for backend_name in VISION_BACKENDS:
        if backend_name not in BACKENDS:
            continue
        if not BACKENDS[backend_name].get("key"):
            continue
        if health_tracker.is_cooled_down(backend_name):
            continue
        try:
            result = await asyncio.wait_for(
                asyncio.to_thread(_call_vision_backend, backend_name, messages, max_tokens, ide), timeout=60.0
            )
