"""Test unified backend registry — canonical modules as single source of truth.
...
def test_proxy_backend_set_is_shared_from_backend_registry():
    assert http_caller.GFW_BACKENDS is backends_constants.GFW_BACKENDS
    assert "google_flash" in backends_constants.GFW_BACKENDS
    ...
    for pool_name, tiers in POOLS.items():
        for tier_name, pool_backends in tiers.items():
            for b in pool_backends:
                if b not in backends_registry.BACKENDS:
...
def test_direct_backends_registered():
    ...
    missing = [b for b in DIRECT_BACKENDS if b not in backends_registry.BACKENDS]
    assert not missing, f"DIRECT_BACKENDS not in BACKENDS: {missing}"


def test_vision_backends_registered():
    missing = [b for b in backends_constants.VISION_BACKENDS if b not in backends_registry.BACKENDS]
    assert not missing, f"VISION_BACKENDS not in BACKENDS: {missing}"


def test_thinking_backends_registered():
    missing = [b for b in backends_constants.THINKING_BACKENDS if b not in backends_registry.BACKENDS]
    assert not missing, f"THINKING_BACKENDS not in BACKENDS: {missing}"


def test_strong_models_registered():
    missing = [b for b in backends_constants.STRONG_MODELS if b not in backends_registry.BACKENDS]
    assert not missing, f"STRONG_MODELS not in BACKENDS: {missing}"


def test_gfw_backends_registered():
    missing = [b for b in backends_constants.GFW_BACKENDS if b not in backends_registry.BACKENDS]
    assert not missing, f"GFW_BACKENDS not in BACKENDS: {missing}"


def test_weak_backends_registered():
    missing = [b for b in backends_constants.WEAK_BACKENDS if b not in backends_registry.BACKENDS]
...
def test_ide_sources_non_empty():
    assert len(backends_constants.IDE_SOURCES) >= 10, "IDE_SOURCES should cover major IDEs"
...
def test_backend_has_capability_unknown_backend():
    assert backend_utils.backend_has_capability("nonexistent_backend_xyz", "code") is False


def test_backend_registry_marks_configured_vision_caps():
    assert backend_utils.backend_has_capability("vision_joycaption", "vision")


def test_is_weak_backend():
    assert backend_utils.is_weak_backend("chat_ubi") is True
    assert backend_utils.is_weak_backend("pollinations") is True
    assert backend_utils.is_weak_backend("llm7") is True
    assert backend_utils.is_weak_backend("scnet_ds_flash") is False
    assert backend_utils.is_weak_backend("nonexistent") is False
...
def test_infer_key_pool_provider():
    assert backend_utils.infer_key_pool_provider("groq_llama70b", {}) == "groq"
    assert backend_utils.infer_key_pool_provider("or_deepseek_r1", {}) == "openrouter"
    assert backend_utils.infer_key_pool_provider("github_gpt4o", {}) == "github"
    assert backend_utils.infer_key_pool_provider("unknown_backend", {}) == ""
...
def test_backends_dict_non_empty():
    assert len(backends_registry.BACKENDS) >= 150, "BACKENDS should have 150+ entries"


def test_every_backend_has_url():
    missing = [k for k, v in backends_registry.BACKENDS.items() if not v.get("url")]
    assert not missing, f"Backends without URL: {missing}"


def test_every_backend_has_fmt():
    missing = [k for k, v in backends_registry.BACKENDS.items() if not v.get("fmt")]
    assert not missing, f"Backends without fmt: {missing}"


def test_every_backend_has_model():
    missing = [k for k, v in backends_registry.BACKENDS.items() if not v.get("model")]
...
def test_detect_caps_returns_list():
    caps = backend_utils.detect_caps("longcat_chat")
    assert isinstance(caps, list)
    assert len(caps) >= 1


def test_vision_backends_have_vision_cap():
    for b in backends_constants.VISION_BACKENDS:
        caps = backend_utils.detect_caps(b)
...
def test_detect_vendor_known():
    assert backend_utils.detect_vendor("https://api.groq.com/openai/v1") == "Groq"
    assert backend_utils.detect_vendor("https://api.longcat.chat/anthropic/v1/messages") == "LongCat"
    assert (
        backend_utils.detect_vendor("https://generativelanguage.googleapis.com/v1beta/openai/chat/completions")
        == "Google"
    )


def test_detect_tier_returns_string():
    tier = backend_utils.detect_tier("https://api.longcat.chat/anthropic/v1/messages", "longcat")
    assert isinstance(tier, str)
    assert tier != "Unknown"


def test_detect_protocol():
    assert backend_utils.detect_protocol("anthropic") == "Anthropic"
    assert backend_utils.detect_protocol("openai") == "OpenAI"


# ── VISION_BACKENDS single source ────────────────────────────────────────────────


def test_vision_handler_uses_constants_source():
    import vision_handler

    assert vision_handler.VISION_BACKENDS is backends_constants.VISION_BACKENDS, (
        "vision_handler.VISION_BACKENDS should be backends_constants.VISION_BACKENDS"
    )
...
def test_router_v3_uses_constants_ide_sources():
    import router_v3

    assert router_v3.IDE_SOURCES is backends_constants.IDE_SOURCES, (
        "router_v3.IDE_SOURCES should be backends_constants.IDE_SOURCES"
    )
...
def test_skills_injector_uses_constants_strong_models():
    import skills_injector

    assert skills_injector.STRONG_MODELS is backends_constants.STRONG_MODELS, (
        "skills_injector.STRONG_MODELS should be backends_constants.STRONG_MODELS"
    )
...
def test_backend_is_enabled_default_true():
    assert backend_utils.is_enabled("scnet_ds_flash") is True
    assert backend_utils.is_enabled("nonexistent_backend") is True


def test_backend_set_enabled_and_is_enabled():
    backend_utils.set_enabled("scnet_ds_flash", False)
    assert backend_utils.is_enabled("scnet_ds_flash") is False
    backend_utils.set_enabled("scnet_ds_flash", True)
    assert backend_utils.is_enabled("scnet_ds_flash") is True
