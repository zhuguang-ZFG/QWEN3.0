...
def _helpers():
    from backend_probe_loop import (
        _record_backend_profile,
        _record_backend_telemetry,
        _record_health_tracker,
    )
...
def test_failed_failure_paths(monkeypatch):
    ht, bp, obs = _inject_all(monkeypatch)
    assert (
        _record()({"backend": "nvidia", "status": "failed", "error_code": 503, "error": "timeout", "latency_ms": 999})
        is True
    )
