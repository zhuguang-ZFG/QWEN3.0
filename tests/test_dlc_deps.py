...
def test_lookup_token_from_db_returns_device_id_when_found():
    ...
    with patch("device_logic.db.connect", return_value=mock_ctx):
        result = lookup_device_id_by_token(token)
...
def test_lookup_token_from_db_returns_none_when_not_found():
    ...
    with patch("device_logic.db.connect", return_value=mock_ctx):
        result = lookup_device_id_by_token("unknown-token")

    assert result is None


def test_lookup_token_from_db_returns_sentinel_on_exception():
    """DB errors → DB_UNAVAILABLE (env fallback path)."""
    with patch("device_logic.db.connect", side_effect=Exception("DB unavailable")):
        result = lookup_device_id_by_token("any-token")

    assert result is DB_UNAVAILABLE


def test_lookup_token_from_db_returns_sentinel_when_table_missing():
    """OperationalError (no such table) → DB_UNAVAILABLE."""
    with patch(
        "device_logic.db.connect",
        side_effect=sqlite3.OperationalError("no such table: v2_device_token"),
    ):
        result = lookup_device_id_by_token("any-token")
...
def test_verify_prefers_db_over_env(monkeypatch):
    ...
    with patch("dlc_api.deps.lookup_device_id_by_token", return_value="db-device"):
...
def test_verify_falls_back_to_env_when_db_unavailable(monkeypatch):
    """When DB returns DB_UNAVAILABLE, env var should still work."""
...
def test_verify_rejects_invalid_token_not_in_db_or_env(monkeypatch):
    ...
    with patch("dlc_api.deps.lookup_device_id_by_token", return_value=None):
