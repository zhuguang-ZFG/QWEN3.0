...
PROM_PAYLOAD = {
    ...
    "data": {
        ...
        "result": [
            ...
            {
                "metric": {"phase": "session_memory.daemon.start", "le": "10000.0"},
                "value": [1710000000, "1"],
            },
            {
                "metric": {"phase": "session_memory.daemon.start", "le": "+Inf"},
                "value": [1710000000, "1"],
            },
        ],
    },
...
def test_parse_prometheus_vector_extracts_phase_bucket_counts():
    ...
    assert samples == [
        ...
        ("session_memory.daemon.start", 10000.0, 1),
        ("session_memory.daemon.start", float("inf"), 1),
    ]
...
def test_summarize_phase_buckets_recommends_thresholds_above_max_bucket():
    ...
    assert by_phase["session_memory.daemon.start"] == {
        "phase": "session_memory.daemon.start",
        "samples": 1,
...
def test_render_markdown_outputs_chinese_report_table():
    ...
    assert "session_memory.daemon.start" in report
