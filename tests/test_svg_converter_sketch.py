...
class TestExtractSvgPaths:
    ...
    def test_skeleton_mode_prunes_spurs(self):
        # Binary T-shape: vertical bar with a short left spur.
        binary = np.zeros((20, 20), dtype=np.uint8)
        binary[4:16, 10] = 255  # vertical bar
        binary[8, 8:10] = 255  # 2-pixel spur
        paths_pruned, _ = _extract_svg_paths(binary, 1.0, 1, skeletonize=True, spur_length_threshold=5)
        paths_raw, _ = _extract_svg_paths(binary, 1.0, 1, skeletonize=True, spur_length_threshold=0)
        assert len(paths_pruned) <= len(paths_raw)

    def test_min_stroke_length_filters_fragments(self):
        binary = np.zeros((20, 20), dtype=np.uint8)
        binary[10, 2:18] = 255  # long stroke
        binary[5, 2:4] = 255  # tiny 2-pixel fragment
        paths, _ = _extract_svg_paths(binary, 1.0, 1, skeletonize=True, min_stroke_length=10.0)
        assert len(paths) == 1
