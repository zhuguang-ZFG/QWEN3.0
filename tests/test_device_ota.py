...
def _app_ota_client(monkeypatch):
    ...
    monkeypatch.setattr(device_ota, "device_access", lambda _conn, _account, _device_id: True)
    monkeypatch.setattr(
        device_ota,
        "get_device_row",
        lambda _conn, _device_id: {"firmware_ver": "1.0.0"},
    )
    monkeypatch.setattr(device_ota.registry, "get", lambda _device_id: None)
...
def test_app_ota_check_no_release(monkeypatch):
    ...
    response = client.get("/device/v1/ota/check", params={"device_id": "dev-1"}, headers={"Authorization": "Bearer test-token"})
    assert response.status_code == 200
    data = response.json()
...
def test_app_ota_check_available_not_selected(monkeypatch):
    ...
    client.post(
        "/device/v1/ota/release/criteria",
        headers={"Authorization": "Bearer test-private-token"},
        params={"name": "canary_verified", "passed": "true"},
    )
    client.post(
        "/device/v1/ota/release/criteria",
        headers={"Authorization": "Bearer test-private-token"},
        params={"name": "safety_review", "passed": "true"},
    )
    client.post(
        "/device/v1/ota/deploy/v2.0.0",
        headers={"Authorization": "Bearer test-private-token"},
...
def test_app_ota_rollback_removes_device(monkeypatch):
    ...
    client.post("/device/v1/ota/start", headers={"Authorization": "Bearer test-token"}, json={"device_id": "dev-1"})
