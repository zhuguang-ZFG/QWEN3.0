...
def _mk_entry(
    device_id: str, eid: str, mtype: MemoryType, key: str, value: str, confidence: float = 1.0, disabled: bool = False
) -> MemoryEntry:
    return MemoryEntry(
        id=eid,
        device_id=device_id,
...
class TestRecallPlannerHints:
    def test_preferences_in_hints(self):
        store = MemoryStore()
        store.create(_mk_entry("dev-1", "p1", MemoryType.PREFERENCE, "favorite_color", "blue", confidence=0.8))
    ...
    def test_procedure_confidence_in_hints(self):
        ...
        store.create(
            _mk_entry(
                "dev-1",
                "c1",
                MemoryType.PROCEDURE_CONFIDENCE,
                "conf_creative",
    ...
    def test_disabled_memory_not_recalled(self):
        store = MemoryStore()
        store.create(_mk_entry("dev-1", "p1", MemoryType.PREFERENCE, "color", "red", confidence=0.8, disabled=True))
        hints = recall_planner_hints(store, "dev-1")
        assert len(hints["preferences"]) == 0

    def test_low_confidence_not_recalled(self):
        store = MemoryStore()
        store.create(_mk_entry("dev-1", "p1", MemoryType.PREFERENCE, "color", "red", confidence=0.3))
        hints = recall_planner_hints(store, "dev-1")
        assert len(hints["preferences"]) == 0

    def test_hard_safety_not_overridden(self):
        store = MemoryStore()
        store.create(_mk_entry("dev-1", "s1", MemoryType.PREFERENCE, "max_feed", "5000", confidence=0.9))
        hints = recall_planner_hints(store, "dev-1")
...
class TestPreferredFeed:
    def test_valid_feed_recalled(self):
        store = MemoryStore()
        store.create(_mk_entry("dev-1", "f1", MemoryType.PREFERENCE, "feed_rate", "800", confidence=0.8))
    ...
    def test_feed_clamped_to_safe_range(self):
        ...
        store = MemoryStore()
        store.create(_mk_entry("dev-1", "f1", MemoryType.PREFERENCE, "feed_rate", "5000", confidence=0.8))
        feed = get_preferred_feed_for_device(store, "dev-1")
        assert feed == 3000.0
    ...
    def test_feed_clamped_to_min(self):
        ...
        store = MemoryStore()
        store.create(_mk_entry("dev-2", "f2", MemoryType.PREFERENCE, "feed_rate", "50", confidence=0.8))
