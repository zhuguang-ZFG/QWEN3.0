...
def http_post(url, payload, timeout=20):
    ...
    req = urllib.request.Request(url, data=data, headers={"Content-Type": "application/json", "User-Agent": "LiMa/2.0"})
...
def test_chat_ubi():
    r = http_post(
        "https://ch.at/v1/chat/completions", {"messages": [{"role": "user", "content": "2+2=? One number only."}]}
    )
    data = json.loads(r)
