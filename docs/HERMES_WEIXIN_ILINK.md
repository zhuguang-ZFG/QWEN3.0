...
- **不能像普通好友一样「转发名片」推荐给朋友**；访客 **不能** 搜索 `xxx@im.bot`，也 **不能** 通过 `/邀请` 的 liteapp 链接接入**同一台** VPS 机器人（日志已证实好友消息不会进桥）。
- **当前可行**：管理员本人微信 ↔ 已扫码绑定的 ClawBot 对话；**多人**请用 `https://chat.donglicao.com` 或每人自建 ClawBot/OpenClaw。
