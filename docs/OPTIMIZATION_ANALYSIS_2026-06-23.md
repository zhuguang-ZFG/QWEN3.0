...
- `routing_engine.py`（调用链复杂）+ `routing_selector/`（后端排名）+ `routing_ml.py`（已移除，ML 路由实验结束）
- 每次请求都要做意图分析 + 后端选择 + 健康检查

**建议**:
- 后端健康状态应缓存（TTL 5-10s），避免每次请求都 probe
- `routing_ml.py` 已移除；当前路由决策基于 `routing_selector/` 的启发式/历史分数，后续若引入 ML 需重新设计热加载方案
...
| ~~`coding_backend_scorer.py` 等 7 个 cold 模块被标记为 dead code，但通过 `ImportError` 动态导入~~ | findings.md ORPHAN-1 | **已删除**（2026-06-26 P9 清理）：`coding_backend_scorer.py`、`eval_*.py`、`periodic_coding_eval.py`、`orchestrate*.py`、`backends_constants_code_tools.py`、`context_pipeline/{code_scanner,semantic_code_retrieval,code_context_injection,graph_retrieval,reranking}.py` |
...
- **累计减重**: ~2,400 行（截至 2026-06-23）
- **2026-06-26 追加**: P9 编码能力退役 shim 删除再减约 5,600 行（含 orchestrate/eval/periodic_coding/coding_backend_scorer/backends_constants_code_tools/上下文模块/小智兼容层及测试）
