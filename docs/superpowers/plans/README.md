...
| **P4** | **提示词系统强化** | `prompts/` 模板注册表（P4-1 ✅）、`routing/semantic_router.py` 规则预筛基线（P4-2 ✅）、`models/structured_outputs/` Pydantic 校验基线（P4-3 ✅）、promptfoo 回归测试（P4-4 ⏸️）、语义缓存（P4-5 ⏸️）、LangGraph 状态可视化（P4-6 ⏸️） | **P4-1~P4-3 已完成；P4-4~P4-6 未开始** | P4-3 当前为 schema + validator 基线，Instructor patched client 预留接口 |

---

## 推荐下一步

1. **P4-4 提示词回归测试**：集成 promptfoo 或类似工具，对 `prompts/layers.yaml` 做快照回归。
2. **P4-5 语义缓存层**：对高频问题做 embedding 相似度缓存，降低后端调用成本。
3. **P4-6 编排管线状态可视化**：如后续恢复多模型编排，可用 LangGraph 状态机替代原 `orchestrate*.py`。
4. **P4-3 后续**：如需要 LLM-native 结构化输出，再安装 `instructor` 并接入 `models/structured_outputs/instructor_client.py`。
