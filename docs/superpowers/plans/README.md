...
> 已完结或已被取代的方案已迁移至 `docs/archive/superpowers-plans-2026-06/`。
> 追踪状态截至 **2026-06-28**。
...
1. **P4-6 后续**：如恢复多模型编排，可将 `pipeline_graph.py` 的节点/边映射到 LangGraph 状态机。
2. **系统级下一步**：`docs/superpowers/plans/LiMa_QWEN3_系统增强细化方案_v3_20260624.md` 中 P5/P6 建议项（可观测性增强、多租户限流）待评估。
3. **灰度观测**：在灰度环境开启 `LIMA_INSTRUCTOR_INTENT_ENABLED=1` 与 `LIMA_SEMANTIC_CACHE_ENABLED=1`，观测命中率、意图准确率与延迟，再决定是否默认开启。
