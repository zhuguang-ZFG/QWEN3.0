...
The current runtime truth is: VPS Server/Worker APIs are deployed, LiMa Code completed a public real-machine worker smoke, FRP `8088` chat is healthy again after local router startup hardening, and the latest `server.py` decomposition slice (`chat_models.py`) is deployed and smoke-verified.
...
- `server.py` decomposition remains active after extracting lifespan and chat request models; backend configuration consolidation and `key_pool.py` integration remain the active architecture backlog.
...
The main `task_plan.md` project phases are complete. 2026-05-22 Superpowers execution plans are checkbox-reconciled. Later plan files may still contain draft checkboxes, but current runtime status is calibrated in `STATUS.md`, `docs/EXECUTION_PLAN.md`, `docs/LIMA_MEMORY.md`, and `progress.md`; latest VPS evidence includes backup `/opt/lima-router/backups/chat-models-extract-20260524_113220`, HTTPS exact `deploy_https_ok_1134`, FRP exact `lima-chat-models-frp-ok`, and worker preflight `ready=true`.
