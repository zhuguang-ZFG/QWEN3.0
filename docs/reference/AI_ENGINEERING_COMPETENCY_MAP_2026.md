...
| Streaming and async | User experience and throughput depend on streaming, async tool work, and background jobs. | OpenAI/Anthropic streaming paths, async backends, background memory daemon, agent task store, and Device Gateway WebSocket queues exist. | Expand async observability around tool calls, stream footers, and device in-flight tasks. |
...
| MLOps and deployment | Reliability comes from guardrails, monitoring, drift checks, rollback, and deployment evidence. | VPS deployment records, health checks, nginx/security notes, docs status, Device Gateway HA boundary, and GitHub push discipline exist. | Add drift/regression dashboards and shared-store HA gates before multi-machine production. |
...
5. Add drift/cost/latency dashboards before multi-process or multi-machine HA.
