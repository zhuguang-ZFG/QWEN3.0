...
> Scope: map the 12 concepts to LiMa Server, LiMa, and future hardware
> control work.
...
|---|---|---|---|
| Prompt engineering | Prompt templates, task contracts, context preflight, tool instructions, identity handling, and prompt-injection defense. | Server prompt staging, IDE/source hints, context digest, LiMa task prompts, and prompt tests exist. | Make prompt contracts versioned data with regression snapshots for high-risk routes. |
...
| Agentic AI and tool calling | Agents are bounded workers with tools, ownership, evidence, and stop gates. | LiMa worker, MCP tools, Device Gateway tasks, and sub-agent governance are documented. | Add per-tool risk class, approval metadata, and audit events before broad tool expansion. |
...
| Token and cost management | FinOps is prompt compression, model routing, quotas, budgets, and telemetry. | Token-safe refresh, context budget checks, route scoring, quota-state handling, and provider key custody exist. | Add per-task budget envelopes and cost attribution to LiMa worker runs. |
...
| `deepcode-cli` / LiMa | Prompt contracts, tool calling, local worker UX, MCP connector discipline, coding eval evidence. |
