...
| `https://github.com/QwenLM/Qwen3-TTS.git` | Multilingual streaming TTS / custom voice / voice design / voice clone provider candidate | Apache-2.0 source reference with 0.6B/1.7B models, multilingual streaming and non-streaming TTS, natural-language voice control, DashScope API, vLLM-Omni examples, fine-tuning, and evaluation notes | Treat as a backend/provider experiment only. Model/API terms, explicit consent, voice-cloning policy, latency/GPU budget, audio retention, and abuse prevention must pass before enabling. |
...
2. Voice hardware extension:
   ...
   - optional Qwen3-TTS experiments for multilingual streaming TTS, custom
     voice, voice design, or voice clone after model/API terms, consent,
     retention, and serving-budget gates;
