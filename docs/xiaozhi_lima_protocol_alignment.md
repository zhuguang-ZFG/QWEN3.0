...
> **状态更新（2026-06-18）**：本文档 §3 覆盖矩阵基于 Phase 0 审计时（2026-06-10）的状态，此后 `routes/xiaozhi_compat/*` 已完成大量实现。当前真实状态请优先参考 [`docs/XIAOZHI_SERVER_RETIREMENT_CHECKLIST_CN.md`](XIAOZHI_SERVER_RETIREMENT_CHECKLIST_CN.md)。下文仍保留原始矩阵以供历史对照，但 P0/P1/P2 清单已在 §7 更新。
...
| 项 | 结论 |
| --- | --- |
| OpenAPI 业务操作数 | 27 个唯一业务操作（去重后）+ 1 个 `/health` |
| 直接覆盖的兼容 REST | 27/27，位于 `routes/xiaozhi_compat/*`（挂载入口 `routes/xiaozhi_v1_compat.py`） |
| 功能等价但路径不同的实现 | 已全部补充 OpenAPI 别名：`/auth/login`、`/devices/{id}/members`、`/voiceprints/{id}`、`PUT /transfers/{id}/cancel` |
| 兼容 router 前缀 | `/api/v1` |
| route_registry 注册状态 | 默认关闭；设置 `LIMA_XIAOZHI_COMPAT_ENABLED=1` 后挂载 |
...
### P0：Phase 0 必须补齐（已 majority 完成）

1. ✅ 在 `routes/route_registry.py` 已支持 `LIMA_XIAOZHI_COMPAT_ENABLED=1` 时挂载 `routes.xiaozhi_v1_compat.router`。
2. ✅ 补 Auth：`POST /auth/register`、`POST /auth/sms-verification`、`GET /auth/me` 已实现。
3. ✅ 补 Device：`POST /devices/register`、`GET /devices`、`GET /devices/{deviceId}`、`POST /devices/{deviceId}/unbind` 已实现。
4. ✅ 补 Task：`GET /devices/{deviceId}/tasks`、`GET /tasks/{taskId}` 已实现。
5. ⏳ 统一任务下行帧：明确 `motion_task` 与 `task_dispatch` 的兼容关系，并写入协议文档/固件适配。
6. ✅ 为已实现 REST 兼容接口补集成测试，覆盖 login -> bind -> submit task -> pending/approve/reject（见 `tests/test_xiaozhi_v1_compat_p0.py`、`tests/test_xiaozhi_v1_compat_p1.py`）。

### P1：迁移闭环（已完成）

1. ✅ `POST /auth/account/delete`：软删账号、解绑设备、匿名化成员/声纹。
2. ✅ `DELETE /voiceprints/{voiceprintId}`：软删除声纹并清除 member.voiceprint_id。
3. ✅ Transfer 全套接口：申请、待处理、接受、取消。
4. ✅ `PUT /devices/{deviceId}`：更新设备信息。
5. ✅ `GET /devices/{deviceId}/self-checks`：自检历史查询。
6. ✅ Supply 查询/更新：`GET/PUT /devices/{deviceId}/supplies`。
...
1. 明确 `motion_task` 与 `task_dispatch` 下行帧兼容策略并同步固件。
2. 建立 display/audio/speech/ocr/camera/perception 能力族独立审批门。
3. 真机端到端回归（唤醒 → VAD → ASR → LLM → TTS → 播放 + 声纹注册/识别）。

验收标准：

- OpenAPI 27 个业务操作均已实现或补全别名。
- P0/P1 REST 接口在本地 FastAPI app 中全部可路由。
- motion 任务可从 REST 创建，经 Device Gateway 下发/排队，并由 `motion_event` 更新状态。
- `route_registry.py` 在 `LIMA_XIAOZHI_COMPAT_ENABLED=1` 时正确加载兼容 router。
