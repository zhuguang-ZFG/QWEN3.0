...
| 关注点 | 来源 |
|---|---|
| 生产请求管道 | `docs/REQUEST_PIPELINE_AUTHORITY.md` |
| 路由引擎设计 | `docs/ROUTING_ENGINE_DESIGN.md` |
| 后端/模型目录 | `docs/MODEL_CATALOG.md` |
| 免费模型证据和策略 | `docs/FREE_MODEL_ROUTING_STATUS.md` |
| 产品子模块边界 | `docs/ESP32S_XYZ_MANAGEMENT.md` |
...
| 产品模式兼容性 | 完成 | `esp32S_XYZ` 提交 `a8d98e3` 在 Edge-B 和 Edge-C `motion_task` 模式和示例中接受 `route_policy`。 |
| LiMa 子模块指针 | 完成 | 主仓库提交 `423bf3e` 将 `esp32S_XYZ` 推进到模式兼容版本。 |
...
3. 更新 LiMa 代码/文档/测试。
4. 推进 LiMa 子模块指针。
5. 在 `STATUS.md`、`progress.md` 和 `docs/LIMA_MEMORY.md` 中记录验证证据。
...
证据应从 `docs/FREE_MODEL_ROUTING_STATUS.md` 或新的有日期准入报告链接。路由池编辑不得与无关重构混合。
...
- 当证据更改后端排名或准入状态时更新 `docs/FREE_MODEL_ROUTING_STATUS.md`。
- 仅在路由所有权或生产管道更改时更新 `docs/REQUEST_PIPELINE_AUTHORITY.md`。
