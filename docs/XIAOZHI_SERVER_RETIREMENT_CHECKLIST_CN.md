...
**当前判定**：小智服务器**仍不能退役**。阶段 3 已完成阿里云 NLS 凭证接入、数字人接入 LiMa，并跑通真实凭证闭环冒烟；目前仅剩 **真机端到端回归** 未执行。
...
从未在真 ESP32 设备上跑通完整链路。当前已补充 `scripts/firmware_hardware_gate.py`，用于在没有 ESP-IDF 或真实设备凭据时明确阻塞，不把未构建/未真机烟测误报为通过。
