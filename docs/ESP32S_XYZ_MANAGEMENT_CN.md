...
云端绘图/写字路径与设备工作区规则见 `docs/ARCHITECTURE.md` 与 `device_gateway/path_workspace.py`。
...
对于影响此产品的 LiMa 后端更改，还需验证相关的主仓库后端测试和公共/私有端点冒烟测试，然后将证据记录在 `STATUS.md` 和 `progress.md` 中。
