...
- `esp32S_XYZ` 子模块指针已推进到 `a4cab61`，包含 Edge-C 硬契约和 Edge-B/Edge-C route_policy 可选 `backend` 字段。
...
- esp32S_XYZ 提交 `a8d98e3` 添加 `route_policy` 至 Edge-B/Edge-C 模式；2026-06-15 后续推进到 `a4cab61`，包含 Edge-C 硬契约和 route_policy backend 字段。
