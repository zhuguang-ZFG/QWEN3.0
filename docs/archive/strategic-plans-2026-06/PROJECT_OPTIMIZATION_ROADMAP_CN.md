...
LiMa 现在是一个多后端 AI 路由服务器和设备云端控制平面。当前的优先级不再是添加更多通用聊天功能，而是让 AI 绘图/写字机工作流变得可靠、可观测和安全，同时保持现有的 OpenAI 兼容 API 可用。

已关闭的关键切片：

- `docs/AI_DRAWING_WRITING_MODEL_ROUTING_GUIDE_CN.md` 定义了绘图/写字模型角色、准入门、切换规则、设备感知输入、安全、可观测性和发布验证。
...
- 2026-06-15 Hardware AI Phase 1 M5–M8 已关闭：recovery/memory/support/OTA 实现、review 修复、452 聚焦测试通过、VPS 部署验证通过。
- 2026-06-15 Edge-C `motion_task` 已把 `route_policy` 从软约束提升为硬契约：固件 Edge-C schema required 化，固件 DeviceServer 和云端 `xiaozhi_compat` 两条下行链路均补齐 `route_policy`。
- 2026-06-15 `route_policy.backend` 字段已贯通：`resolve_device_route_policy()` 复用 `get_preferred_backend()` 填充真实后端，典型值包括 `deterministic`、`dashscope_wanx` 和 `opencv_contour`；固件 Edge-B/Edge-C schema 接受可选 `backend` 字段。
- 2026-06-18 `draw_generated` 自然语言主链路已接入 `device_draw_handler`（`task_draw_params.py`）；生图失败为 `draw_failed`，证据见 `docs/testing/draw_generated_task_creation.tdd.md`。
- 2026-06-15 死区代码与文档清理已完成：Anthropic 兼容层退役、死文件删除、过时文档归档、findings 轮转。
- `esp32S_XYZ` 子模块已推进到包含 Edge-C 硬契约与 backend 字段的 `a4cab61`。

阶段 1 已关闭：云端 `validate_route_policy`、制品证据与假 U8 闭环已覆盖；物理 U1 运动固件侧拒绝由 `esp32S_XYZ` 子模块跟进（见 `findings.md` U1RP-4，状态 Accepted）。

## 下一阶段执行主线（2026-06-16）

当前 G1–G4 主线（AI→Motion 发布门、模型准入复跑、证据边界瘦身、启动/部署不确定性降低）已基本完成，后续优化方向由本路线图持续更新。

| 目标 | 内容 | 对应阶段 | 交付物 |
|---|---|---|---|
| G1 AI→Motion 发布门 | 从用户请求追踪到 `motion_event` 终态或阻断证据 | 阶段 1 + 阶段 5 | 首份真实 `docs/release_evidence/YYYY-MM-DD-*.md` 报告 |
| G2 设备模型准入复跑 | 让 `device_draw`、`device_vector`、`device_write`、`device_control` 的准入依据可复跑 | 阶段 2 | `docs/model_admission/` 有日期报告与 eval 证据 |
| G3 证据边界瘦身 | 继续删除冷区，但必须有 CodeGraph / 文本搜索 / 测试证据 | 文档系统 + 瘦身路线 | `CODEBASE_SUBSYSTEM_TIER_CN.md`、`CODEBASE_COLD_PRUNE_PRIORITY_CN.md` 更新 |
| G4 启动与部署不确定性降低 | 拆解 lifespan 启动耗时，区分 ready 与 warming | 阶段 4 + 阶段 5 | 启动阶段日志、健康语义和部署 smoke 记录 |
...
| 流 | 负责区域 | 当前状态 | 下一步成果 |
|---|---|---|---|
| 设备任务路由 | `device_gateway/` | 路由角色和 `route_policy` 已存在；backend 字段已贯通 | 假 U8 消费并报告路由策略 |
| 绘图/写字模型策略 | `docs/AI_DRAWING_WRITING_MODEL_ROUTING_GUIDE_CN.md` | 云端策略已文档化 | 首次有日期的图像/矢量准入报告 |
| 产品固件/模式 | `esp32S_XYZ/` | Edge-B/C 模式接受路由策略 | U8 适配器验证并记录路由策略 |
| 运动安全 | `path_validator.py`、产品 U1/U8 | 验证器存在，物理门仍分离 | 模拟器和假设备发布门成为强制性 |
| 通用 LLM 路由 | `routing_engine.py`、`router_v3/`、`routing_selector/` | 现有聊天/IDE 路由引擎保持活跃 | 设备角色停止依赖通用聊天池行为 |
...
晋升规则：后端只有在从特定角色的准入报告链接其有日期的证据后才能进入热绘图/写字路由。
...
目标：保留公共 OpenAI 兼容 API，而不让通用聊天回归控制设备安全。

步骤：

1. 保持 `routing_engine.route()` 作为权威聊天/IDE 路由入口。
2. 按表面拆分路由测试：聊天/编码、设备、运维、产品集成。
3. 保持提供商健康/冷却/预算故障在日志和指标中可见。
4. 避免将本地 Windows 代理后端提升到 VPS 优先路由，除非 VPS 进程能通过文档化的拓扑访问它们。
...
4. 在 `STATUS.md`、`progress.md` 和 `docs/LIMA_MEMORY_CN.md` 中存储发布证据；长报告放在 `docs/release_evidence/` 下。
...
| `docs/LIMA_MEMORY_CN.md` | 持久跨会话记忆 |
| `docs/REQUEST_PIPELINE_AUTHORITY_CN.md` | 生产路由所有权 |
| `docs/AI_DRAWING_WRITING_MODEL_ROUTING_GUIDE_CN.md` | 设备模型路由策略 |
| `docs/ESP32S_XYZ_MANAGEMENT_CN.md` | 产品子模块边界 |
| `docs/PROJECT_OPTIMIZATION_ROADMAP_CN.md` | 活跃全项目路线图 |
...
- 为未来代理不应重新发现的跨会话事实更新 `docs/LIMA_MEMORY_CN.md`。
...
5. 当物理运动行为更改时存在硬件证据。
6. `STATUS.md`、`progress.md` 和 `docs/LIMA_MEMORY_CN.md` 包含持久结果。
7. 仅暂存、提交和推送相关文件。
