...
**draw_generated**（App/语音提交；云端投影为 `run_path` 下发）:
...
**draw_generated 云端构建（2026-06-18）**:

1. `intent` 将「画…」解析为 `capability=draw_generated`。
2. `task_draw_params.build_draw_generated_params`：
   - prompt 形如 SVG path → `path_pipeline.render_svg_task`（无 AI）；
   - 否则 → `device_draw_handler.handle_device_draw`（预设 / 万相简笔画 / OpenCV 矢量化）→ `render_svg_task(svg_path)`。
3. 失败时任务 `status=failed`，`error.code=draw_failed`。
4. 成功时 `capability` 投影为 `run_path`，设备只执行 `path` + `feed`。

详见 [`testing/draw_generated_task_creation.tdd.md`](testing/draw_generated_task_creation.tdd.md)。
