...
当前阶段 1 尚未完全关闭的剩余项：假 U8 消费 route_policy、设备制品记录路由证据、U1 拒绝未知策略。
...
按优先级排序，作为 M9 及以后里程碑的候选切片：

1. **M9 假 U8 消费 `route_policy`（阶段 1 收尾）**
   - 目标：让产品 fake U8 在收到下行 `motion_task` 时解析、记录并断言 `route_policy` 字段。
   - 预期文件：`esp32S_XYZ/tools/fake_lima_u8/app.py`、`esp32S_XYZ/tools/fake_lima_u8/tests/test_app.py`、Edge-C 模式示例。

2. **M10 LiMa 设备制品记录路由证据（阶段 1 收尾）**
   - 目标：任务生成、阻止、验证失败、恢复路径都在 `device_artifacts/` 中留下 route_role/backend/capabilities 证据。
   - 预期文件：`device_gateway/tasks.py`、`device_artifacts/`、`tests/test_device_gateway_model_routing.py`。

3. **M11 设备模型准入报告脚手架（阶段 2 启动）**
   - 目标：创建 `docs/model_admission/YYYY-MM-DD-device-drawing-writing.md` 模板和最小可复现评估命令。
   - 预期文件：`docs/model_admission/TEMPLATE.md`、`scripts/eval_device_model_role.py` 或等效可重复命令列表。

4. **M12 设备配置文件路由输入（阶段 3 启动）**
   - 目标：把设备 profile 字段引入 `resolve_device_route_policy()`，缺失数据保守降级。
   - 预期文件：`device_gateway/profiles.py`、`device_gateway/tasks.py`、匹配的路由和存储测试。

5. **M13 AI 到运动发布证据模板（阶段 5 启动）**
