...
日期: 2026-06-25 (扩展 CQ-087；已同步编码能力退役)
...
| 关注点 | 权威模块 | 遗留/兼容外观 | 备注 |
|--------|----------|---------------|------|
| 后端注册 | `backends_registry/` 包 + `backends_constants.py` | — | 检测助手位于 `backend_utils.py` |
| 意图分析 | `routing_intent.py` | — | `analyze_intent()` 统一意图分类 |
| 场景分类 | `routing_classifier.py` | — | `classify()` → request_type (ide/chat/vision); `classify_scenario()` → scenario (coding/chat) |
...
> **已知 bypass 收敛：** 历史审计文档已归档，当前生产路径以 `routing_engine.route()` 为准。

```text
1. identity_guard    — 身份识别短路 (→ 直接返回)
2. classify          — request_type (ide/chat/vision)
3. classify_scenario — scenario (coding/chat)
4. skill_store       — 技能记忆召回 → recalled_backend
5. retrieval_injection — 知识图谱/向量上下文注入（按需）
6. complexity        — 请求复杂度评估
7. router_v3.select_backends → routing_selector.select — 后端排名
8. skills_injector  — Skills 注入到 messages
9. context_compressor — (可选) 长对话压缩
10. speculative      — (简单请求) 推测性并行调用
11. routing_executor.execute — 按序/并行执行 + fallback
12. route_post_process — 后处理 (correlation/evidence/feedback)
13. feedback_bridge  — 闭环反馈记录
```
...
- `tests/test_retrieval_injection.py` — 检索注入
...
- 对等测试：工厂阶段 vs 生产跟踪 仅用于 `/v1/chat/completions`（`/v1/messages` 已退役）
