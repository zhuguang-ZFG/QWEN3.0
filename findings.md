...
- `pytest --tb=short -q`：**4179 passed, 3 skipped, 2 deselected, 0 failed**
...
| AUDIT-5 | 可观测性 | O1 健康探活/O2 query 脱敏/O3 告警评估器/O4 审计日志/O7 X-Request-Id/O8 结构化日志默认开启/O9 日志轮转/O10 错误聚合/O11 /health 信息泄漏 | HIGH 全关闭；O5/O6 延后 |
...
- **延后项（架构级改造，需独立排期）**：O5（审计 append-only 化）、O6（OpenTelemetry 接入）
- 状态：**AUDIT-5 HIGH 批次已全部关闭**（O1/O2/O3/O4/O7/O8/O9/O10/O11）。
