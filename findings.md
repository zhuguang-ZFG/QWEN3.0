...
| CQ-014 | Open | Post-route integrations moved to `route_post_process.py` (`routing_engine.py` now ~372 lines). Admin HTML/JS moved to `routes/admin_ui.py` (`routes/admin.py` now ~330 lines). Oversized files remain: `smart_router.py`, `server.py`, `http_caller.py`, `health_tracker.py`. | Continue slice extraction per `docs/CQ014_POST_ROUTE_SLICE.md` and `docs/CQ014_ADMIN_UI_SLICE.md`. |
