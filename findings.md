...
| AUDIT-5 | 可观测性 | O1 健康探活/O2 query 脱敏/O4 审计日志 + O8 结构化日志默认开启 | HIGH 全关闭；O3/O5/O6/O7/O9/O10/O11 延后 |
...
- **延后项（架构级改造，需独立排期）**：O3（告警规则评估器，需定时消费循环）、O5（审计 append-only 化）、O6（OpenTelemetry 接入）、O7（X-Request-Id 响应头，需全路由注入）、O9（日志轮转）、O10（错误聚合）、O11（/health 信息泄漏）
- 状态：**AUDIT-5 HIGH 批次与 O8 已关闭**（O1/O2/O4/O8）。
