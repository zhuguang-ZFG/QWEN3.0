...
| AUDIT-3 | 提示词 | P1 中文注入/P2 输出 guardrail 接入/P3 注入阻断 BLOCK | HIGH 全关闭 |
| AUDIT-4 | 容错率 | F2 reaper/F3 背压 Semaphore/F5 非流式降级/F6 monotonic 修复 + F1 客户端重试 | 第一批+F1 关闭（F4/F7 延后） |
| AUDIT-5 | 可观测性 | O1 健康探活/O2 query 脱敏/O4 审计日志+假审计修复 | HIGH 全关闭 |
| AUDIT-6 | API 契约 | A1 禁文档/A2 error 类型统一/A3 错误码表 | HIGH 全关闭 |
| AUDIT-7 | 部署运维 | D1 CI pytest/D2 部署审批/D3 Litestream 异地 | HIGH 全关闭（D4 延后） |
...
- 延后项：P4（LiMa 层覆盖客户端 system，需评估 OpenAI 兼容性影响）、P5（版本标记注释）、P6（IDE 值字符过滤）、P7（上下文摘要关键词过滤）——均为 LOW 级别。
- 状态：**AUDIT-3 HIGH 批次已关闭**（P1/P2/P3）。
...
- **延后高风险项（需专门集成测试，避免核心路径回归）**：
  - ~~F1（客户端层瞬时错误重试）~~ ✅ 已完成
  - F4（httpx.Client 连接池复用）：见 AUDIT-8 P4（已完成）
  - F7（device/MQTT 启动降 WARM）：需验证降级后设备功能不影响主服务
- 状态：**AUDIT-4 F1/F2/F3/F5/F6 已关闭**；F4 见 AUDIT-8-P4；F7 标注延后。
...
- **延后项（架构级改造，需独立排期）**：O3（告警规则评估器，需定时消费循环）、O5（审计 append-only 化）、O6（OpenTelemetry 接入）、O7（X-Request-Id 响应头，需全路由注入）、O8/O9（结构化日志默认开/日志轮转，运维配置）
- 状态：**AUDIT-5 HIGH 批次已关闭**（O1/O2/O4）。
...
- **延后项（大范围端点改造）**：A4（统一响应信封，涉及所有 device 端点）、A5（REST 风格统一）、A6（Pydantic 收口）、A7（/v1/models 动态生成）、T1/T2（测试补齐）
- 状态：**AUDIT-6 HIGH 批次已关闭**（A1/A2/A3）。
...
- **延后项**：D4（依赖 hash pinning，需 pip-compile 生成锁文件并验证构建）、D5-D9（limit_req_zone 版本化/镜像钉版本/资源限制/TLS 硬化/回滚覆盖 .env，多为运维配置层）
- 状态：**AUDIT-7 HIGH 批次已关闭**（D1/D2/D3）。D1 是恢复 CI 测试门禁的最高杠杆修复。
...
- **延后项（核心路径改造，需专门性能/集成测试）**：P1（instructor 同步 LLM 异步化）、P5（embedding 异步化+LRU）、P6/P7（共享状态+多 worker）
- 状态：**AUDIT-8 P3/P2/P4 已关闭**；P1/P5/P6/P7 延后（核心路径，需配套集成测试）。
...
- **延后项**：A1（SVG 净化，需 lxml 依赖）、W2（移除 query 参数 token 注入，需前后端协同）、W3（僵尸会话心跳清理）、I2（autohanding 连接池复用）
- 状态：**AUDIT-11 HIGH 批次已关闭**（I1/W1）。
