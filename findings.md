...
- `pytest --tb=short -q`：**4173 passed, 3 skipped, 2 deselected, 0 failed**
...
| AUDIT-5 | 可观测性 | O1 健康探活/O2 query 脱敏/O4 审计日志/O7 X-Request-Id/O8 结构化日志默认开启/O9 日志轮转/O10 错误聚合/O11 /health 信息泄漏 | HIGH 全关闭；O3/O5/O6 延后 |
...
| AUDIT-8 | 性能 | P1 instructor 缓存/降级 + P2 intent 去重 + P3 缓存 WAL + P4 httpx 连接池复用 + P5 embedding LRU + P8 投机执行优化 + P9 退役热路径清理 | P1/P2/P3/P4/P5/P8/P9 关闭（P6/P7 延后） |
...
- **延后项（架构级改造，需独立排期）**：O3（告警规则评估器，需定时消费循环）、O5（审计 append-only 化）、O6（OpenTelemetry 接入）
- 状态：**AUDIT-5 HIGH 批次与 O7/O8/O9/O10/O11 已关闭**（O1/O2/O4/O7/O8/O9/O10/O11）。
...
- 状态：**AUDIT-8 P1/P2/P3/P4/P5/P8/P9 已关闭**；P6/P7 延后。
