...
| CQ-056 | Closed | Eval apply plus WeChat owner-handler follow-up initially risked malformed Agent Task records, repeat promotion if the promoted memory aged out of the recent window, and 500s for malformed eval/apply JSON. Review fixed `/code-task` to use the formal Agent Task creation contract, made `apply_promotion()` deeply idempotent, added stable eval/apply 400 responses, and strengthened P1.4 fake-device assertions. Focused tests return 95 passed, 2 skipped; P1.4 stability returns 8 passed, 1 skipped; full suite returns 1359 passed, 10 skipped. VPS deployment passed remote compile, restart/health, ops/eval smoke, public eval revision, and online smoke 12/12; latest backup is `/opt/lima-router/backups/review-fix-json-20260525_124238/runtime-before.tgz`. | `/v1/ops/eval/apply` is now explicit and idempotent, but it is still a behavior-changing operator action. Keep it behind private auth, audit records, and human release review; do not trigger it from chat, guest channels, or automatic learning. |
