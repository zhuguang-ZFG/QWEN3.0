...
| CQ-014 | Open | Post-route integrations moved to `route_post_process.py` (`routing_engine.py` now ~372 lines). Oversized files remain: `smart_router.py`, `server.py`, `routes/admin.py`, `http_caller.py`, `health_tracker.py`. | Continue slice extraction per `docs/CQ014_POST_ROUTE_SLICE.md`. |
