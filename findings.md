...
## 2026-06-05 OpenCode E2E 真实 VPS 验证 + Bug 修复

### 验证结果 (2026-06-05 16:35 CST)

**10/11 PASS, 平均编码得分 92**

| Step | Result | Score | Notes |
|------|--------|-------|-------|
| health_check | PASS | - | 1614ms |
| coding_context_overflow_recovery | PASS | 80 | missing cors pattern |
| coding_ide_code_explain | PASS | 75 | forbidden pattern: database|DB |
| coding_multi_file_edit | PASS | 100 | clean |
| coding_reasoning_effort | PASS | 100 | clean |
| coding_streaming_tool_args | FAIL | 0 | groq_gptoss 格式不匹配 |
| coding_tool_call_generation | PASS | 80 | missing "path" pattern |
| coding_typescript_typefix | PASS | 100 | clean |
| tool_calls | PASS | - | tools=1, correct_name=True |
| streaming | PASS | - | SSE 正常, TTFB 543ms |
| streaming_tools | FAIL | - | Groq 断开连接 |

### 修复的 Bug (4 个)

| Bug | 文件 | 根因 | 修复 |
|-----|------|------|------|
| OpenCode prefer 被覆盖 | `routes/chat_handler_dispatch.py` | `model="lima"` 设置 `prefer="longcat_lite"`，OpenCode prefer 逻辑不生效 | 改为 `if prefer in (None, "longcat_lite"): prefer = OPENCODE_PREFERRED_BACKEND` |
| bytes 赋值错误 | `routes/opencode_direct_stream.py` | `_build_body` 返回 bytes，`body["tool_choice"] = ...` 失败 | 先 json.loads 修改后再 json.dumps |
| stream_response 参数不匹配 | `routes/chat_stream.py` | VPS 版本过旧缺少 `has_tools` 参数 | 上传最新版本 |
| scnet_ds_flash 不支持 tool_calls | `opencode_config.py` | 默认后端不支持 tool_calls | 改为 `groq_gptoss` |

### 发现的架构问题

1. **TOOL_CAPABLE_BACKENDS 与 OPENCODE_PREFERRED_BACKEND 不一致**：`scnet_ds_flash` 是默认 OpenCode 后端，但不在 `TOOL_CAPABLE_BACKENDS` 列表中
2. **VPS 文件同步不完整**：`deploy_vps_bundle.py` 未上传 `opencode_direct_stream.py`、`opencode_config.py` 等关键文件
3. **GitHub token 过期**：VPS 上的 `GITHUB_TOKEN` 返回 401 Bad credentials
4. **Groq 流式 tool_calls 不稳定**：`groq_gptoss` 在流式 tool_calls 时断开连接

### 已解决：tool_calls 适配层 + LongCat OpenAI 格式支持

**解决方案**：
1. **text_tool_extractor 适配层**：已实现 `text_tool_extractor.py`，支持从文本中提取 JSON 格式的 tool_calls
2. **opencode_direct_stream 集成**：当后端在 `TEXT_TOOL_BACKENDS` 中时，注入 tool system prompt 并从响应中提取 tool_calls
3. **LongCat OpenAI 格式**：新增 `longcat_openai` 后端，使用 `https://api.longcat.chat/openai/v1/chat/completions`，支持原生 tool_calls

**验证结果 (2026-06-06 00:53 CST)**：

**API 层面：11/11 PASS, 平均编码得分 92**

**真实 OpenCode 客户端验证 (2026-06-06 01:00 CST)**：
- ✅ 连接 VPS 成功 (lima-1.3)
- ✅ tool_calls: bash 工具执行 `Get-ChildItem` 成功
- ✅ read_file: 读取 Makefile 成功
- ✅ 流式响应: tokens=51577

| Step | Result | Score | Notes |
|------|--------|-------|-------|
| health_check | PASS | - | 1485ms |
| coding_context_overflow_recovery | PASS | 80 | missing cors pattern |
| coding_ide_code_explain | PASS | 90 | too long: 1036 chars |
| coding_multi_file_edit | PASS | 100 | clean |
| coding_reasoning_effort | PASS | 100 | clean |
| coding_streaming_tool_args | PASS | 100 | clean |
| coding_tool_call_generation | PASS | 80 | missing "path" pattern |
| coding_typescript_typefix | PASS | 100 | clean |
| tool_calls | PASS | - | tools=1, correct_name=True |
| streaming | PASS | - | SSE 正常, TTFB 2561ms |
| streaming_tools | PASS | - | tools=1, args_valid=True |
