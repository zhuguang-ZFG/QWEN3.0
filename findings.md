...
| CQ-031 | Partial | M6-S1/S2/S4 add local observability events, in-memory metrics, snapshot queries, and docs. Review fixed event-object redaction so raw prompt/key/cookie-like metadata cannot survive inside `LiMaEvent` or key-pool details. | Complete M6-S3 by wiring hot paths gradually after the event model has stayed stable for one slice. |
